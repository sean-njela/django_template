...
```bash
  [1/27] project_name (My Awesome Project): Django Template 
  [2/27] project_slug (django_template): 
  [3/27] description (Behold My Awesome Project!): Django template
  [4/27] author_name (Daniel Roy Greenfeld): Sean Njela
  [5/27] domain_name (example.com): devopssean.netlify.app
  [6/27] email (sean-njela@devopssean.netlify.app): 
  [7/27] version (0.1.0):
